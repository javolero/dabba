...
SV_WC_Framework_Bootstrap::instance()->register_plugin( '4.0.0', __( 'WooCommerce Social Login', 'woocommerce-social-login' ), __FILE__, 'init_woocommerce_social_login', array( 'minimum_wc_version' => '2.2', 'backwards_compatible' => '4.0.0' ) );
...
class WC_Social_Login extends SV_WC_Plugin {
	...
	private function frontend_includes() {
		...
		require_once( $this->get_plugin_path() . '/includes/frontend/class-wc-social-login-frontend.php' );
	...
	private function admin_includes() {
		...
		require_once( $this->get_plugin_path() . '/includes/admin/class-wc-social-login-admin.php' );
	...
	public function add_admin_notices() {
		...
		if ( class_exists( 'ITSEC_Tweaks' ) ) {
			...
			if ( $this->is_plugin_settings() && isset( $ithemes_security_settings['long_url_strings'] ) && $ithemes_security_settings['long_url_strings'] ) {
				...
				$this->get_admin_notice_handler()->add_admin_notice(
					esc_html__( 'Oops, looks like iThemes Security is set to Filter Long URLs. This is likely to cause a conflict with Social Login -- please disable that setting for optimal functionality.', $this->text_domain ),
