...
class WC_Social_Login_Opauth {
	...
	public function init_config() {
		...
		if ( ! empty( $_GET['wc-social-login-error'] ) ) {
			...
			wc_add_notice( sprintf( __( '%sERROR%s: Couldn&#8217;t register you&hellip; please contact us if you continue to have problems.', WC_Social_Login::TEXT_DOMAIN ), '<strong>', '</strong>' ), 'error' );

		} elseif ( ! empty( $_GET['wc-social-login-restricted-role-error'] ) ) {

			wc_add_notice( __( 'Oops, it looks like you may already have an account&hellip; please log in to link your profile.', WC_Social_Login::TEXT_DOMAIN ), 'error' );
		}
