 ...
 * @since 1.6.0
 */
class WC_Social_Login_Provider_VK extends WC_Social_Login_Provider {
	...
	public function __construct( $base_auth_path ) {
		...
		$this->id                = 'vk';
		$this->title             = __( 'VK', WC_Social_Login::TEXT_DOMAIN );
		$this->strategy_class    = 'SVVK';
		$this->color             = '#567ca4';
		$this->require_ssl       = false;
		$this->internal_callback = 'oauth2callback';

		$this->notices = array(
			'account_linked'         => __( 'Your VK account is now linked to your account.', WC_Social_Login::TEXT_DOMAIN ),
			'account_unlinked'       => __( 'VK account was successfully unlinked from your account.', WC_Social_Login::TEXT_DOMAIN ),
			'account_already_linked' => __( 'This VK account is already linked to another user account.', WC_Social_Login::TEXT_DOMAIN ),
			'account_already_exists' => __( 'A user account using the same email address as this VK account already exists.', WC_Social_Login::TEXT_DOMAIN ),
		);
	...
	public function get_description() {
		...
		return sprintf( __( 'Need help setting up and configuring VK? %sRead the docs%s', WC_Social_Login::TEXT_DOMAIN ), '<a href="http://docs.woothemes.com/document/woocommerce-social-login-create-social-apps#vk">', '</a>' );
	}
	...
	public function init_form_fields() {
		...
		$this->form_fields['id']['title']     = __( 'Application ID', WC_Social_Login::TEXT_DOMAIN );
		$this->form_fields['secret']['title'] = __( 'Secure Key', WC_Social_Login::TEXT_DOMAIN );
	}
	...
	public function get_default_login_button_text() {
		...
		return __( 'Log in with VK', WC_Social_Login::TEXT_DOMAIN );
	}
	...
	public function get_default_link_button_text() {
		...
		return __( 'Link your account to VK', WC_Social_Login::TEXT_DOMAIN );
	}
