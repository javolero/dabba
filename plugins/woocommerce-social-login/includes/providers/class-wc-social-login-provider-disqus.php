...
class WC_Social_Login_Provider_Disqus extends WC_Social_Login_Provider {
	...
	public function __construct( $base_auth_path ) {
		...
		$this->id                = 'disqus';
		$this->title             = __( 'Disqus', WC_Social_Login::TEXT_DOMAIN );
		...
		$this->notices = array(
			'account_linked'         => __( 'Your Disqus account is now linked to your account.', WC_Social_Login::TEXT_DOMAIN ),
			'account_unlinked'       => __( 'Disqus account was successfully unlinked from your account.', WC_Social_Login::TEXT_DOMAIN ),
			'account_already_linked' => __( 'This Disqus account is already linked to another user account.', WC_Social_Login::TEXT_DOMAIN ),
			'account_already_exists' => __( 'A user account using the same email address as this Disqus account already exists.', WC_Social_Login::TEXT_DOMAIN ),
		);
	...
	public function get_description() {
		...
		return sprintf( __( 'Need help setting up and configuring Disqus? %sRead the docs%s', WC_Social_Login::TEXT_DOMAIN ), '<a href="http://docs.woothemes.com/document/woocommerce-social-login-create-social-apps#disqus">', '</a>' );
	}
	...
	public function get_default_login_button_text() {
		...
		return __( 'Log in with Disqus', WC_Social_Login::TEXT_DOMAIN );
	}
	...
	public function get_default_link_button_text() {
		...
		return __( 'Link your account to Disqus', WC_Social_Login::TEXT_DOMAIN );
	}
